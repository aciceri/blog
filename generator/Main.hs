...
baseContext =
  headVersionField "git-head-commit" Commit
...
embedAsciinema text =
  let macro = do
      ...
      embed name = "<div id='cast-" ++ name  ++ "'></div><script>AsciinemaPlayer.create('/casts/" ++ name ++ ".cast', document.getElementById('cast-" ++ name ++ "'));</script>"
   in streamEdit macro embed text
